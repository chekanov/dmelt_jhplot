 ...
 * @author  Leigh Brookshaw, S.Chekanov 
 */
...
aram f Font
   ...
   */
     ...
     public void parseText(Graphics g) {
         ...
         if (text.startsWith("#bar{") || text.startsWith("#sqrt{")) text=" "+text; 
...
// chekanov
            case '#':
                      ...
                      current.c  = null;
                   //   current.y += (int)((double)(current.getDescent(g))*sub_offset+0.5);
                      current.id=1; // this is bar or sqrt 
                      break;
         ...
         for(int i=0; i<list.size(); i++) {
              ...
              if(ts.s != null) { 
                    ...
                    String string=ts.toString();
                     ...
                     if (ts.id == 0) { 
                        lg.drawString(string,ts.x+xoffset,ts.y+yoffset);
                        ff=ts.f;
                     }


// draw string with bar 
              if (bar == 1) {
               lg.setFont(ff);
               lg.drawString(string,ts.x+xoffset-3*off,ts.y+yoffset);
               FontMetrics fm = lg.getFontMetrics ();
               int w = fm.stringWidth(string);
               int h = fm.getHeight() -  (int)(fm.getHeight()/3.0);

               Graphics2D g2 = (Graphics2D) lg;
               g2.setStroke(new BasicStroke(1));
               if (h>10) g2.setStroke(new BasicStroke(2)); 
               if (h>24) g2.setStroke(new BasicStroke(3));
               if (h>32) g2.setStroke(new BasicStroke(4));
 
               //lg.setStroke(thindashed); 
               // System.out.println(h); 
               lg.drawLine(ts.x+xoffset-3*off,ts.y+yoffset-h,ts.x+xoffset+w-3*off,ts.y+yoffset-h);
              } 
...
// draw sqrt
             ...
             if (sqrt == 1) {
               ...
               int h = fm.getHeight() -  (int)(fm.getHeight()/3.0);
                ...
                //lg.setStroke(thindashed); 
               // System.out.println(h); 
               int yspace=(int)(h*0.1);
               if (yspace<1) yspace=1;
               // horizontal top bar
               lg.drawLine(ts.x+xoffset-3*off,ts.y+yoffset-h-yspace,ts.x+xoffset+w-3*off,ts.y+yoffset-h-yspace);
               // vertical large bar
               lg.drawLine(ts.x+xoffset-3*off,ts.y+yoffset-h-yspace,ts.x+xoffset-3*off-off,ts.y+yoffset-h+h+yspace);
               lg.drawLine(ts.x+xoffset-4*off-off,(int)(ts.y+yoffset-h+0.5*h),ts.x+xoffset-4*off-off+off,ts.y+yoffset-h+h+yspace);
               // small tip
               lg.drawLine((int)(ts.x+xoffset-4*off-1.3*off),(int)(ts.y+yoffset-h+0.5*h),ts.x+xoffset-4*off-off,(int)(ts.y+yoffset-h+0.5*h)); 
                 ...
                 if (ts.id == 1 )  { 
                     ...
                     // 1/2 character backspace
                     if (string.equals("bs1")) {
                                    back=1;
//ssss
                                    xoffset=xoffset-6*cwidth-(int)(0.5*cwidth);
                                 //   System.out.println(xoffset);                                  

                  }

                  if (string.equals("bs2")) {
                                    back=1;
                                    xoffset=xoffset-7*cwidth;
                                 //   System.out.println(xoffset);                                  
...
/*
// draw line 
  //             System.out.println(stringOverline);
                if (startOverline>-1 &&  endOverline>-1 &&
                      endOverline>startOverline ) {
                int swidth=getWidth(lg);
                int shight=getHeight(lg)-getHeight(lg)/5;
               // System.out.println( shight );
                // shight=10;
                System.out.println("Overline "+stringOverline);
                int start=startOverline*swidth; 
                int end=endOverline*swidth;
             //   lg.drawLine(start,shight+xoffset,end,shight+xoffset);
                   lg.drawLine(50,100,100,100);  
                 }
