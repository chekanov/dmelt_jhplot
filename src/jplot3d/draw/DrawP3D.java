...
public class DrawP3D {
	...
	/**
	         * Creates scatter plot for P3D. At this moment, it's ony functional
	...
	// direction test
	     ...
	     // plot last the most closest to observer
	        for (int ii=jpoint.length-1; ii>=0; ii--) {
	         ...
	         float y1=((float)jpp.getY()-yi) *yfactor-10;
	         float z1=((float)jpp.getZ()-zi) *zfactor-10; 
	         float dx=(float)jpp.getDX()*xfactor;
	         float dy=(float)jpp.getDY()*yfactor;
	         float dz=(float)jpp.getDZ()*zfactor;


	         int sis=(int)(0.15*jpp.getSymbolSize()*sf.projector.get2DScaling());
	            // note: jpp.getSymbolColor() - fill color
	            // jpp.getSymbolSize() - line width
	            DrawPlain2.run(5, x1, y1, z1, dx, dy ,dz, jpp.getSymbolColor(), sis, sf);
	            DrawPlain2.run(1, x1, y1, z1, dx, dy ,dz, jpp.getSymbolColor(), sis, sf );
	             DrawPlain2.run(2, x1, y1, z1, dx, dy ,dz, jpp.getSymbolColor(), sis, sf );
	             DrawPlain2.run(3, x1, y1, z1, dx, dy ,dz,  jpp.getSymbolColor(), sis, sf );
	               DrawPlain2.run(4, x1, y1, z1, dx, dy ,dz, jpp.getSymbolColor(), sis, sf );
	                DrawPlain2.run(0, x1, y1, z1, dx, dy ,dz, jpp.getSymbolColor(), sis, sf);
