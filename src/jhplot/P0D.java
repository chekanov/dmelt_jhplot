...
/**
 * A container to hold data points in one dimension. All numbers are expected to
 * be in double precision. It extends ArrayList and adds many new features for
 * data manipulation. The class does not have graphical option (use H1D to show
 * the data or methods of this class which transform P0D to a H1D histogram).
 * 
 * <p>
...
public class P0D extends DoubleArrayList implements Serializable {
	...
	/**
	 * Construct a copy from a P0D. If the last argument is true, a shallow copy
	...
	/**
	 * Construct a P0D from a file. Use "#" or "*" for comments. each number
			...
			while ((line = br.readLine()) != null) {
				...
				if (!line.startsWith("#") && !line.startsWith("*")) {
					...
					try {
						...
						jhplot.utils.Util.ErrorMessage("Error in reading the line "
								+ Long.toString(mm + 1));
						return 3;
					}
					mm++;
	...
	/**
	 * Read P0D from a GZiped file. It can read URL if the string starts from
	...
	public void toFile(String name) {
		...
		try {
			...
			for (int i = 0; i < size(); i++) {
				String x = ((Double)get(i)).toString();
				tx.println(x);
			}
			f1.close();
	...
	public void writeBinary(String name) {
		BinaryFile.writeDoubleArray(new File(name), getArray(), "BIG_ENDIAN");
	 ...
	 * 
	 * @return new P0D object
	 */
	public P0D readSerialized(String name) {

		return (P0D) jhplot.io.Serialized.read(name);

	}

	/**
	 * Read a P0D from a binary file (big endian by default). Old elements will
	...
	public String toString() {
		...
		String tmp = "P0D " + title + "\n";

		for (int i = 0; i < size(); i++) {
			String x = ((Double)getQuick(i)).toString();
			tmp += x + "\n";
		}
	...
	/**
	 * Return H1D histogram with P0D content. Histogram range is defined by Min
	 * and Max values.
	 * 
	 * @param bins
	...
	/**
	 * Fill a P0D container from an array. If it is not empty, add values will
	 * be appended.
	 * 
	 * @param values
	...
	/**
	 * Set an integer array. Old content will be lost. It will be automatically
	 * transformed to the double.
	 * 
	 * @param array
	 *            array used to fill P0D
	 */
...
/**
	 ...
	 * @param max
	 *            index
	 * @return same P0D with removed range
	 */
	 ...
	 * 
	 * @param p0d
	 *            Input P0D container for operation
	 * @param what
	 ...
	 * @return original P0D after the operation.
	 */

	public P0D oper(P0D p0d, String what) {
		return oper(p0d, this.title, what);
	}
	 ...
	 * @param scale
	 *            Scale factor
	 * @return P0D after the operation.
	 */
	public P0D operScale(double scale) {
		for (int i = 0; i < size(); i++)
			set(i, getQuick(i) * scale);
		return this;
	 ...
	 * @return P0D after shift.
	 * */
	public P0D operShift(double shift) {
		for (int i = 0; i < size(); i++)
			set(i, getQuick(i) + shift);
		return this;
	 ...
	 * @return P0D after shift and scale.
	 **/

	public P0D operShiftAndScale(double shift, double scale) {
		for (int i = 0; i < size(); i++)
			set(i, (getQuick(i) + shift) * scale);
		return this;
	...
	public P0D oper(P0D p0d, String title, String what) {
		...
		// first check them
		if (size() != p0d.size()) {
			jhplot.utils.Util.ErrorMessage("Sizes of the P0Ds are different!");
			return this;
		}
		...
		if (what.equals("-")) {
			for (int i = 0; i <size(); i++)
				set(i, getQuick(i) - p0d.getQuick(i));
			return this;
		}
	...
	public String getStatString() {
		...
		cern.colt.list.DoubleArrayList a= new cern.colt.list.DoubleArrayList(elements() );
		bin.addAllOf(a);
		return bin.toString();
	...
	/**
	 * Print the P0D container to a Table in a separate Frame. The numbers are
	 ...
	 * c1.setAutoRange();
	 * 
	 * P0D p0 = new P0D("Normal distribution"); p0.randomNormal(1000, 0.0, 1.0);
	 * 
	 * c1.setNameX("X"); c1.setNameY("Y");
	 * 
	 * // make copy and transform. String func1 = "x*cos(x)+2"; P0D p01 =
	 * p0.copy(func1); // p01.func(func1);
	 * 
	 * // make copy and transform. func1 = "exp(x)-2"; P0D p02 = p0.copy(func1);
	 * // p02.func(func1);
	 * 
