 ...
 * permutation vector piv of length m so that A(piv,:) = L*U. In other words,
 * assuming P the permutation Matrix, P*A = L*U. If m < n, then L is m-by-m and
 * U is m-by-n.
 * <P>
...
public class LUDecomposition {
     ...
     * @param A Rectangular matrix @param linpackflag Use Gaussian elimination.
     * Actual value ignored. @return Structure to access L, U and piv. \ public
     * LUDecomposition (Matrix A, int linpackflag) { // Initialize. LU =
    ...
    /*
     * ---------- JAMA code. ---------- *\ \** Return pivot permutation vector
     * @return piv \ public int[] getPivot () { int[] p = new int[m]; for (int i =
     * 0; i < m; i++) { p[i] = piv[i]; } return p; } \** Return pivot
     * permutation vector as a one-dimensional double array @return (double) piv \
     * public double[] getDoublePivot () { double[] vals = new double[m]; for
     * (int i = 0; i < m; i++) { vals[i] = (double) piv[i]; } return vals; } \*
     * ----------------- End of JAMA code. -----------------
     */
    ...
    public double[][] solve(double[][] B) {
        ...
        int nx = B[0].length;
        double[][] Xmat = DoubleArray.getRowsCopy(B, piv); // B.getMatrix(piv,0,nx-1);

        // Solve L*Y = B(piv,:)
        for (int k = 0; k < n; k++) {
            for (int i = k + 1; i < n; i++) {
                for (int j = 0; j < nx; j++) {
                    Xmat[i][j] = Xmat[i][j] - Xmat[k][j] * LU[i][k];
                }
            }
        }
        // Solve U*X = Y;
