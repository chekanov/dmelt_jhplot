 ...
 * 
 * @author Yanto Suryono and Sergei Chekanov
 */
...
public class JSurface extends javax.swing.JComponent {
	...
	public SurfaceModel model; // the parent, Surface Plotter model
	public Projector projector; // the projector, controls the point of view
	public SurfaceVertex[][] vertex; // vertices array
	public  boolean data_available; // data availability flag
	public  boolean interrupted; // interrupted flag
	public  boolean critical; // for speed up
	public boolean printing; // printing flag
	private int prevwidth, prevheight; // canvas size
	private int printwidth, printheight; // print size
	public SurfaceVertex cop; // center of projection 
	private JPopupMenu menu;
     public JMenuItem item1;
     public JMenuItem item2;
    private float scalingFrame=1.0f; // scaling factor after resize 
    private boolean first_frame=true;
    
	public int curve = 0; 
    
	public VectorGraphics  graphics; // the actual graphics used by all private
								// methods
	...
	public boolean plotfunc1, plotfunc2, plotboth;
	public boolean isBoxed, isMesh, isScaleBox, isDisplayXY, isDisplayZ,
			isDisplayGrids;
	public float xmin, xmax, ymin;
	public float ymax, zmin, zmax;
	...
	void setModel(SurfaceModel model) {
		...
		cop = new SurfaceVertex(projector.getDistance()
				* projector.getSinRotationAngle(), projector.getDistance()
				* projector.getCosRotationAngle(), projector.getDistance()
				* projector.getSinElevationAngle());
		cop.setProjector(projector);
		
		//surfaceVertex= new SurfaceVertex(0,0,0);
	...
	class JSurfaceMouseListener extends MouseAdapter implements
		...
		public void mouseWheelMoved(MouseWheelEvent e) {
			...
			if (new_value != old_value) {
				...
				model.setInitScaling(new_value);
				repaint();
			}
		}
		...
		public void mouseDragged(MouseEvent e) {
			...
			if (e.isControlDown()) {
				...
				// limit rotations in case of histograms
	...
	public void paintComponent(Graphics g) {
		...
		if (first_frame == true) {comp_init=getBounds(); 
		                          first_frame=false; 
        ...
        // float s=projector.get2DScaling();
          projector.set2DScaling((model.getInitScaling()*scalingFrame));
		...
		if (data_available && !interrupted) {
			...
			vg.setColor(colors.getBackgroundColor());
			vg.fillRect(0, 0, getBounds().width, getBounds().height);
			if (is3D())
				drawBoxGridsTicksLabels(vg, true);
		}
		interrupted = false;
	}

	private boolean is3D() {
		return (plot_type == PlotType.WIREFRAME || plot_type == PlotType.SURFACE || plot_type == PlotType.BARS);
	}
	...
	private synchronized void draw(Graphics graphics) {
		...
		// graphics.setFont(new Font("Helvetica",Font.PLAIN,fontsize));
                ...
                // chekanov: correct here
		SurfaceVertex.invalidate();

       if (model.isEmpty() == true) {DrawEmpty.run(this); cleanUpMemory(); return;  } // draw empty

       if (model.isP2D() == true)  {
    	   DrawP2D.run(this); cleanUpMemory(); return;  } // draw  container
       
       if (model.isP3D() == true) 
             {DrawP3D.run(this); cleanUpMemory(); return;  } // draw empty
     
       if (model.isH2F2() == true) {
           if (plot_type==PlotType.BARS) {
        	    DrawH2D.run(this);  DrawSurface.run(this); plotfunc1=true; plotfunc2=false;
             } else  {DrawSurface.run(this); plotfunc1=true; plotfunc2=true;};
             cleanUpMemory(); return;  
       }
	...
	private final void drawBase(VectorGraphics g, int[] x, int[] y) {
		...
		g.drawPolygon(x, y, 5);
                ...
                float  wline = model.getPenWidth()*model.getFrameScale();
                //if (wline<1.0f) wline=1.0f; 
		g.setStroke(new BasicStroke(wline));	
	}
	...
	public final void drawBoxGridsTicksLabels(VectorGraphics g2d, boolean draw_axes) {
		...
		Font fontAxis=model.getAxisFont();
		Color fontColorAxes=model.getAxesFontColor();
		double  labelOffsetX=model.getLabelOffsetX();
		Font    fontLabel=model.getLabelFont();
		
		String  Xlabel =  model.getXlabel();
		String  Ylabel =  model.getYlabel();
		String  Zlabel =  model.getZlabel();
		Color    fontColorLabel=model.getLabelColor();
		double labelOffsetY = model.getLabelOffsetY();
		double labelOffsetZ = model.getLabelOffsetZ();
		Color  frameColor=model.getColorModel().getBackgroundColor();
		
		g2d.setFont( scaleFont(fontAxis));
		...
		if (draw_axes) {
			...
			if (projection.x < x[0])
				outString(g2d, (int) (1.05 * (projection.x - x[0])) + x[0],
						(int) (1.05 * (projection.y - y[0])) + y[0], "x",
						Label.RIGHT, TOP, fontAxis, fontColorAxes,0);
			else
				outString(g2d, (int) (1.05 * (projection.x - x[0])) + x[0],
						(int) (1.05 * (projection.y - y[0])) + y[0], "x",
						Label.LEFT, TOP, fontAxis, fontColorAxes,0);
			...
			outString(g2d, (int) (1.05 * (projection.x - x[0])) + x[0],
					...
					Label.CENTER, CENTER, fontAxis, fontColorAxes,0);
		} else {
			factor_x = factor_y = 1;
			projection = projector.project(0, 0, -10);
			x[0] = projection.x;
			...
			if (isBoxed) {
				...
				g2d.setColor(colors.getBoxColor());
				g2d.fillPolygon(x, y, 4);
					
				g2d.setColor(colors.getLineBoxColor());
			...
			for (i = -9; i <= 9; i++) {
				if (isDisplayXY || isDisplayGrids) {
					if (!isDisplayGrids || (i % (t_y / 2) == 0) || isDisplayXY) {
						...
						g2d.drawLine(projection.x, projection.y, tickpos.x,
								tickpos.y);
						if ((i % t_y == 0) && isDisplayXY) {
							tickpos = projector.project(factor_x * 10.5f, i,
									-10);
							if (y_left)
								outFloat(g2d, tickpos.x, tickpos.y,
				...
				if (isDisplayXY) {
					...
					outString(g2d, (int) (tickpos.x * labelOffsetX),
							(int) (tickpos.y * labelOffsetX), Xlabel,
							Label.CENTER, TOP, fontLabel, fontColorLabel,0);

					tickpos = projector.project(factor_x * 14, 0, -10);
					outString(g2d, (int) (tickpos.x * labelOffsetY),
							(int) (tickpos.y * labelOffsetY), Ylabel,
							Label.CENTER, TOP, fontLabel, fontColorLabel,0);
				...
				if (isDisplayZ) {
					tickpos = projector.project(-factor_x * 11.0f,
							factor_y * 11.0f, 10);
					// plotZ
					outString(g2d, (int) (tickpos.x * 1.0f / labelOffsetZ),
							(int) (tickpos.y * 1.0f / labelOffsetZ), Zlabel,
							Label.CENTER, TOP, fontLabel, fontColorLabel,0);
						...
						if (isDisplayGrids && isBoxed) {
							if (i % t_y == 0) {
								...
								tickpos = projector.project(-factor_x * 10, i,
										10);
								g2d.drawLine(projection.x, projection.y,
										tickpos.x, tickpos.y);
							}
							if (i % t_x == 0) {
								projection = projector.project(i,
                ...
                float  wwline = model.getPenWidth()*model.getFrameScale();
                //if (wwline<1.0f) wline=1.0f;
		g2d.setStroke(new BasicStroke(wwline));
	}
	...
	private final void outString(VectorGraphics  g2d, int x, int y, String s,
			int x_align, int y_align, Font f, Color c, int angle) {


                Font ff = scaleFont(g2d.getFont());
		switch (y_align) {
		case TOP:
		...
		switch (x_align) {
		case Label.LEFT:
			text.draw(g2d, (int) x, (int) y);
			break;
		case Label.RIGHT:
			text.draw(g2d, (int) (x - g2d.getFontMetrics(g2d.getFont())
					.stringWidth(stext)), (int) y);
			break;
		case Label.CENTER:
			text.draw(g2d, (int) (x - g2d.getFontMetrics(g2d.getFont())
					.stringWidth(stext) / 2), (int) y);
			break;
		}
	...
	/**
	 * Scale fonts depending o the size of the frame
	 * @param fo
	 * @return
	 */
	...
	private final void outFloat(VectorGraphics g2d, int x, int y, float f,
			int x_align, int y_align, int angle) {

		String s = Float.toString(f);
		s = NumUtil.RemoveZero(s);
		outString(g2d, x, y, s, x_align, y_align, model.getAxisFont(), model.getAxesFontColor(),angle);
	}
	...
	public  final void plotArea(int start_lx, int start_ly, int end_lx,
	...
	public final void drawBoundingRect() {
		...
		ylabels=NumUtil.ZeroRemover(ylabels);
		legend_label=NumUtil.ZeroRemover(legend_label);
		...
		if (isDisplayXY || isDisplayGrids) {
			...
			for (int i = -10; i <= 10; i++) {
				if (!isDisplayGrids || (i % (t_y / 2) == 0) || isDisplayXY) {
					...
					if ((i % t_y == 0) && isDisplayXY) {		
						outString(graphics, contourConvertX(10.9f), yc,
								ylabels[labelindex++], Label.LEFT, CENTER,
								fontAxis, fontColorAxes,0);
				...
				if (isDisplayXY) {
					outString(graphics, (x1 + x2) / 2, contourConvertY(-10.9f - (float)model.getLabelOffsetX()),
							 Xlabel, Label.CENTER, TOP,fontLabel, fontColorLabel,0);			
					outString(graphics, contourConvertX(-10.0f-(float)model.getLabelOffsetY() ),
									contourConvertY(-1.0f), Ylabel, Label.LEFT, CENTER,fontLabel, fontColorLabel,-90);
				}
			}
		}
		...
		if (isDisplayZ) {
			...
			outString(graphics, x2, y2, legend_label[0], Label.LEFT, CENTER,fontAxis, fontColorAxes,0);

			// limit the number of axis numbers to 5
			int ntot=5;
			if (divisions<10) ntot=1;
			if (divisions>9 && divisions<21) ntot=2;
			if (divisions>20 && divisions<31) ntot=3;
			if (divisions>30 && divisions<41) ntot=4;
			if (divisions>40 && divisions<51) ntot=5;
			if (divisions>50)ntot=5;
			...
			for (int i = 1; i <= divisions + 1; i++) {
				...
				if (i%ntot == 0 ) outString(graphics, x2, y, legend_label[i], Label.LEFT, CENTER,fontAxis, fontColorAxes,0);
				lasty = y;
			}
		}
	}
