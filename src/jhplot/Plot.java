...
/**
 * Simple canvas to draw data. The syntax is similar to Matlab (actually
 * matplotlib). It is based on jFreeChart. For more advanced canvaces, look at
 * HPlot, HPlotJa.
 * 
 ...
 * 
 */
public class Plot extends JyplotVanilla implements Serializable {
	 ...
	 * i.e. PNG, JPG, PDF, EPS, SVG). No questions will be asked, an existing
	 ...
	 * </pre>
	 * <p>
	 * Draw a horizontal line at y from xmin to xmax. With the default values of
	 * xmin=0 and xmax=1, this line will always span the horizontal extent of
	 * the axes, regardless of the xlim settings, even if you change them, eg
	 * with the xlim command. That is, the horizontal extent is in axes coords:
	 * 0=left, 0.5=middle, 1.0=right but the y location is in data coordinates.
	 * <p>
	 * kwargs are the same as kwargs to plot (however so far only "linewidth"
	 * and "color" are implemented) and can be used to control the line
	 * properties.
	 * <p>
	 * Eg:
	 * <p>
	 ...
	 * 
	 * @param args
	 *            Main arguments e.g. axhline(y=0, xmin=0, xmax=1
	 * @param kws
	...
	public void axhline(float y, float xmin, float xmax, float linewidth, Color color) {
			...
			Marker start = new ValueMarker(y);
			start.setStroke(new BasicStroke((float)linewidth));
	 ...
	 * <p>
	 */
	public Dataset errorbar(double x[], double y[], double xerr[],  double yerr[], 
			String label, Color color, Color cerr, int capsize, boolean barsabove) {
	...
	/**
	 * Sets the figure size. figure(num=None, figsize=None, dpi=None,
	 * facecolor=None, edgecolor=None, frameon=True)
	 */
	public void figure(PyObject[] args, String[] kws) {
		ArgParser ap = new ArgParser("figure", args, kws, new String[] { "num",
		...
		try {
			figsizetuple = (PyTuple) ap.getPyObject(1, new PyTuple(
					new PyObject[] { new PyFloat(8), new PyFloat(6) }));
			// this.figureWidth = (float) ((PyFloat)
			// figsizetuple.get(0)).getValue();
			// this.figureHeight = (float) ((PyFloat)
			// figsizetuple.get(1)).getValue();
			this.dpi = dpi;
		} catch (Exception e) {
			System.out
					.println("Problem with figsize arguments.  They should be in the form: (8,6)");
			// e.printStackTrace();
		}
	...
	public void xticksa(PyObject[] args, String[] kws) {
		...
		/*
		 * If there are no given labels, create them from the locations
		 */
		...
		if (this.chart != null) {
			NumberAxis xNumberAxis = (NumberAxis) this.chart.getXYPlot()
					.getDomainAxis();
			// SymbolAxis symbolaxis = new SymbolAxis(xNumberAxis.getLabel(),
			// ticklabels);
			// symbolaxis.
		...
		try {
			...
			*/
				...
				System.err.println("Failed to convert "
						+ list.getClass().getName() + " into double[]");
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public void legendtest(PyObject[] args, String[] kws) {
		System.out
				.println("legend(PyObject[] args, String[] kw) currently does not work");
		// try
		// {
		// ArgParser ap = new ArgParser("legend", args, kws, new String[]
		// {"lines", "labels", "loc"});
		//
		// String[] labels = getStringArrayFromPyList( ap.getPyObject(1, null)
		// );
			...
			if (filetype.equalsIgnoreCase("png")) {
				...
				try {
					...
					org.jibble.epsgraphics.EpsGraphics2D g = new org.jibble.epsgraphics.EpsGraphics2D(
							"Example", outputStream, 0, 0, width, height);// #Create
					
					
					/**
					 * 
					 *  a new document with bounding box 0<x=100 and 0<y=100
					 */
					...
					System.err
							.println("Epsgraphics2D.jar present in classpath?");
				}
	...
	/**
	 * Sets the figure size. errorbar(x, y, yerr=None, xerr=None, fmt='b-',
	 * ecolor=None, capsize=3, barsabove=False
	 */
	public Dataset plot(PyObject[] args, String[] kws) {
		ArgParser ap = new ArgParser("plot", args, kws, new String[] { "x",
				"y", "shape", "color", "label", "separateYAxis" });
		...
		return dataset;
		// plot(double[] x, double[] y, Color color, String dataname, double[]
		// yErrorBars, boolean seperateYAxis, boolean yaxisVisible, String
		// rangeLabel, boolean shapesVisible)
		// return plot(x, y, c, label, null, separateYAxis, true, null,
		// shapesVisible);
	 ...
	 * </pre>
	 * 
	 * Make a bar plot with rectangles bounded by left, left+width, bottom,
	 * bottom+height (left, right, bottom and top edges) left, height, width,
	...
	public Dataset bar(PyObject[] args, String[] kws) {
		ArgParser ap = new ArgParser("bar", args, kws, new String[] { "left",
				"height", "width", "bottom", "color", "edgecolor", "linewidth",
				"yerr", "xerr", "ecolor", "capsize", "align", "orientation",
				"log" });
		...
		/*
		 * If only a single array is given, assume it is the height, and create
		 * for the left array [0, 1...n]
		 */
		if (height == null) {
			height = left;
		...
		if (colorObject != null) {
			try {
				...
				// System.out.println("colorObject.getClass().getName():" +
				// colorObject.getClass().getName());
	...
	/**
	 * Extract a java.awt.Color object from some PyObject. This could be a
	 * string, a number, a tuple of numbers.
	 * <p>
	...
	private Color getColorFromPyObject(PyObject pyobject) {
		...
		try {
			if (pyobject instanceof PyList) {
				...
				for (int i = 0; i < parray.__len__(); i++) {
					result[i] = (int) parray.__getitem__(i).__float__()
							.getValue();
				}
				if (result.length == 1) {
			...
			// TODO: fix
			else if (Class.forName("java.awt.Color").isInstance(pyobject)) // ||
																			// pyobject.__class__.__name__.equals("java.awt.Color")
			{
				return ((Color) pyobject.__tojava__(Color.class));
			
				
				/*if 
			   (pyobject instanceof JNumeric.PyMultiarray) {
			...
			*/
				...
				// TODO: fix
				// logger.warn("Failed to convert " + pyobject.toString() + "( "
				// + pyobject.getClass().getName() + ", " +
				// pyobject.__class__.__name__ + ") into A Color constructor");
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	/**
	 * Make a semilog plot with log scaling on the x axis. The args to semilog x
	 * are the same as the args to plot. See help plot for more info. Optional
	 * keyword args supported are any of the kwargs supported by plot or
	 * set_xscale. Notable, for log scaling: basex: base of the logarithm subsx:
	 * the location of the minor ticks; None defaults to autosubs, which depend
	 * on the number of decades in the plot; see set_xscale for details
	 */
	public Dataset semilogx(PyObject[] args, String[] kws) {
		ArgParser ap = new ArgParser("semilogx", args, kws, new String[] {
				"basex", "subsx" });

		/*
		 * This part is not used yet double basex = 10; PyObject _basex =
		 * ap.getPyObject(0, null); if(_basex != null && _basex instanceof
		 * PyFloat) { PyFloat fl = (PyFloat) _basex; basex = fl.getValue(); }
		 */
	...
	public Dataset semilogy(PyObject[] args, String[] kws) {
		ArgParser ap = new ArgParser("semilogy", args, kws, new String[] {
				"basex", "subsx" });
	...
	public Dataset loglog(PyObject[] args, String[] kws) {
		// ArgParser ap = new ArgParser("semilogx", args, kws, new
		// String[]{"basex", "subsx"});
		...
		for (int i = 0; i < ((XYSeriesCollection) dataset).getSeriesCount(); i++) {
			...
			for (int j = 0; j < series.getItemCount(); j++) {
				if (series.getX(j).doubleValue() <= 0
						|| series.getY(j).doubleValue() <= 0) {
	...
	public static void main(String[] args) {
		...
		Jyplot p = new Jyplot();
		PyObject[] x = { new PyFloat(0.1), new PyFloat(1), new PyFloat(10),
				new PyFloat(100), new PyFloat(1000) };
		PyObject[] y = { new PyFloat(10), new PyFloat(6), new PyFloat(260),
				new PyFloat(140), new PyFloat(15000) };
	...
	private boolean isDomainLogarithmic() {
		...
		if (this.chart.getXYPlot().getDomainAxis().getClass().getName()
	...
	private boolean isRangeLogarithmic() {
		...
		if (this.chart.getXYPlot().getRangeAxis().getClass().getName()
				.contains("Logarithmic"))
			return true;
